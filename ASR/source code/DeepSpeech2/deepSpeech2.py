...
from lnRNNCell import BasicRNNCell as lnBasicRNNCell
from lnRNNCell import GRUCell as lnGRUCell
from lnRNNCell import BasicLSTMCell as lnBasicLSTMCell
from dynamic_brnn import build_multi_dynamic_brnn
...
def build_deepSpeech2(args,
    ...
    layer1 = tf.contrib.layers.dropout(layer1, keep_prob=args.keep_prob, is_training=args.is_training)
    print(layer1)
    ...
    layer2 = tf.contrib.layers.dropout(layer2, keep_prob=args.keep_prob, is_training=args.is_training)
    print(layer2)
    ...
    layer3 = tf.contrib.layers.dropout(layer3, keep_prob=args.keep_prob, is_training=args.is_training)
    print(layer3)
    # 4 recurrent layers
    # inputs must be [max_time, batch_size ,...]
    batch_size = tf.shape(layer3)[0]
    feat_size = layer3.get_shape().as_list()[1]
    time = layer3.get_shape().as_list()[2]
    #print(feat_size)
    #print(time)
    layer3 = tf.reshape(
        layer3,
        [batch_size, time, feat_size * 96])
    layer3 = tf.transpose(layer3,perm=[1,0,2])
    args.num_layer = 4
    '''
    if seqLengths != None:
        temp = tf.fill([args.batch_size],2)
        temp2 = tf.fill([args.batch_size],1)
        seqLengths = tf.div(seqLengths,temp)
        seqLengths = tf.add(seqLengths,temp2)
    '''
    fbhs = build_multi_dynamic_brnn(args,time,layer3,cell_fn,seqLengths_cnn)
    return fbhs
    
    #print(layer3)
    '''
    layer4_cell = cell_fn(args.num_hidden, activation=args.activation)
    layer4 = tf.nn.dynamic_rnn(layer4_cell, layer3, sequence_length=seqLengths, time_major=False, dtype = tf.float32) 
    layer4 = tf.layers.batch_normalization(layer4, training=args.isTraining)
    layer4 = tf.contrib.layers.dropout(layer4, keep_prob=args.keep_prob, is_training=args.is_training)

    layer5_cell = cell_fn(args.num_hidden, activation=args.activation)
    layer5 = tf.nn.dynamic_rnn(layer5_cell, layer4, sequence_length=seqLengths, time_major=True, dtype = tf.float32) 
    layer5 = tf.layers.batch_normalization(layer5, training=args.isTraining)
    layer5 = tf.contrib.layers.dropout(layer5, keep_prob=args.keep_prob, is_training=args.is_training)

    layer6_cell = cell_fn(args.num_hidden, activation=args.activation)
    layer6 = tf.nn.dynamic_rnn(layer6_cell, layer5, sequence_length=seqLengths, time_major=True, dtype = tf.float32) 
    layer6 = tf.layers.batch_normalization(layer6, training=args.isTraining)
    layer6 = tf.contrib.layers.dropout(layer6, keep_prob=args.keep_prob, is_training=args.is_training)

    layer7_cell = cell_fn(args.num_hidden, activation=args.activation)
    layer7 = tf.nn.dynamic_rnn(layer7_cell, layer6, sequence_length=seqLengths, time_major=True, dtype = tf.float32) 
    layer7 = tf.layers.batch_normalization(layer7, training=args.isTraining)
    layer7 = tf.contrib.layers.dropout(layer7, keep_prob=args.keep_prob, is_training=args.is_training)
...
class DeepSpeech2(object):
    ...
    def build_graph(self, args, maxTimeSteps):
        ...
        with self.graph.as_default():
            ...
            self.seqLengths_after_cnn = tf.placeholder(tf.int32, shape=(args.batch_size))
            ...
            fbHrs = build_deepSpeech2(self.args, maxTimeSteps, inputXrs, self.cell_fn, self.seqLengths,self.seqLengths_after_cnn)
            #print(fbHrs)
            '''
            if self.seqLengths != None:
                temp = tf.fill([args.batch_size],2)
                temp2 = tf.fill([args.batch_size],1)
                self.seqLengths = tf.div(self.seqLengths,temp)
                self.seqLengths = tf.add(self.seqLengths,temp2)
            ...
            print(logits3d)
            self.loss = tf.reduce_mean(tf.nn.ctc_loss(self.targetY, logits3d, self.seqLengths_after_cnn))
            ...
            self.predictions = tf.to_int32(
                tf.nn.ctc_beam_search_decoder(logits3d, self.seqLengths_after_cnn, merge_repeated=False)[0][0])
            if args.level == 'cha':
